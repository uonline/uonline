...
exports.changeLocation = function(dbConnection, userid, locid) {
	var willBeAttacked = dbConnection.query.sync(dbConnection,
		"SELECT 1 FROM uniusers, locations, monsters "+
		"WHERE uniusers.id = $1 "+
			"AND monsters.location = $2 "+
			"AND RANDOM()*100 <= monsters.attack_chance "+
			"AND NOT EXISTS ("+ //TODO: tests
				"SELECT 1 FROM battle_participants "+
				"WHERE kind = 'monster' AND id = monsters.id) "+
		"LIMIT 1", [userid, locid]).rows.length > 0;
	
	if (willBeAttacked)
	{
		var participants = dbConnection.query.sync(dbConnection,
			'SELECT id, initiative FROM monsters '+
			'WHERE monsters.location = $1', [locid]).rows
			.map(function(row){ return {id: row.id, initiative: row.initiative, kind: 'monster'}; });
		...
		tx.query.sync(tx,
			'UPDATE uniusers SET location = $1, autoinvolved_fm = 1, fight_mode = 1 WHERE id = $2',
			[locid, userid]);
		...
		tx.query.sync(tx,
			'INSERT INTO battle_participants (battle, id, kind, index) VALUES '+
				participants.map(function(p,i) {
					return "("+newBattleId+", "+p.id+", '"+p.kind+"', "+i+")";
				}).join(", "));
	...
	else
	{
		dbConnection.query.sync(dbConnection,
			'UPDATE uniusers SET location = $1 WHERE id = $2',
			[locid, userid]);
	}
