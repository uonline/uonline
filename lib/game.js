...
exports._hitAndGetHealth = function(tx, victimId, victimKind, hunterPower) {
	...
	for (var i=0; i<armor.length; i++)
	{
		...
		{
			var delta = Math.min(hunterPower, item.strength);
			tx.query.sync(tx, 'UPDATE armor SET strength = $1 WHERE id = $2', [item.strength-delta, item.id]);
			hunterPower -= delta;
			break;
		}
...
exports._hit = function(dbConnection, hunterId, hunterKind, victimId, victimKind) {
	...
	var health = exports._hitAndGetHealth(tx, victimId, victimKind, hunter.power);
...
var joinedUserCharacters = userCharacters.join(",");
...
exports.getUserCharacters = function(dbConnection, userIdOrName) {
	...
	var user = dbConnection.query.sync(dbConnection,
		'SELECT '+joinedUserCharacters+' FROM uniusers WHERE '+field+' = $1', [userIdOrName]).rows[0];
...
exports.getUserArmor = function(dbConnection, userid) {
	return dbConnection.query.sync(dbConnection,
		'SELECT name, type, coverage, strength, strength_max '+
		'FROM armor, armor_prototypes '+
		'WHERE armor.owner = $1 AND armor.prototype = armor_prototypes.id', [userid]).rows;
}.async();


var monsterCharacters = [
	'name',
	'level',
	'power',
	'agility',
...
exports.getMonsterPrototypeCharacters = function(dbConnection, id) {
	var result = dbConnection.query.sync(
		dbConnection,
