...
exports._hit = function(dbConnection, hunterId, hunterKind, victimId, victimKind) {
	...
	// trying to find victim's participant
	// maybe he has been killed already?
	//TODO: locking victim
	var victimParticipant = tx.query.sync(tx,
		"SELECT id "+
		...
		"WHERE id = $1 "+
			...
			"AND battle = $3 "+
			"AND side = $4",
		[victimId, victimKind, hunter.battle, 1-hunter.side]).rows[0];
	
	if (victimParticipant === undefined)
	{
		tx.rollback.sync(tx);
		return;
	}
	...
	if (health <= 0)
	{
		exports._leaveBattle(tx, hunter.battle, victimId, victimKind);
...
exports.hitOpponent = function(dbConnection, userid, participantId, participantKind) {
	exports._hit(dbConnection, userid, 'user', participantId, participantKind);
}.async();
