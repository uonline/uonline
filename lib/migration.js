...
var migrationData = [
	...
	[
		...
		['uniusers', 'ALTER TABLE uniusers ALTER COLUMN permissions DROP DEFAULT'],
		...
		['uniusers', "ALTER TABLE uniusers ALTER COLUMN permissions SET DEFAULT 'user'"],
	],
];
...
function justMigrate(dbConnection, revision, for_tables) {
	...
	for (var i=0; i<migration.length; i++)
	{
		...
		if (params[FUNC_NAME_COLUMN] in tables)
		{
		...
		{
			//assume function is SQL query
			var sql = params[FUNC_NAME_COLUMN];
			dbConnection.query.sync(dbConnection, sql);
		}
	}
}
