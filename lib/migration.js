...
exports.migrate = function(dbConnection, opts, callback) {
	...
	async.waterfall([
		...
		function(current, innerCallback) {
			var i = current+1;
			async.whilst(
				function() {return i<=opts.dest_revision;},
				function(veryInnerCallback) {
					if (opts.verbose)
					{
						console.log('Migrating '+(opts.table ? '<'+opts.table+'> ' : '')+'to revision '+i);
					}
					justMigrate(dbConnection, i++, opts.table, veryInnerCallback);
				},
				innerCallback
			);
		},
		function(innerCallback) {
			...
			{
				exports.setRevision(dbConnection, opts.dest_revision, innerCallback);
			}
		},
		function(innerCallback) {
			if (opts.verbose)
