...
{
	dbConnection.query(
		'SELECT count(*) AS result FROM uniusers WHERE lower(username) = lower($1)',
		[username],
...
exports.sessionInfoRefreshing = function(dbConnection, sessid, sess_timeexpire, asyncUpdate, callback) {
	if (!sessid)
	{
		return { sessionIsActive: false };
	}

	var userdata = dbConnection.query.sync(dbConnection,
	...
	return {
		...
		admin: (userdata.rows[0].permissions === config.PERMISSIONS_ADMIN),
	...
	async.waterfall(
		[
			function (innerCallback) {
				dbConnection.query(
					'SELECT salt, hash FROM uniusers WHERE lower(username) = lower($1)',
					[username],
					function(error, result) {
						if (!!result && result.rows.length === 0)
						{
