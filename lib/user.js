...
{
	dbConnection.query(
		'SELECT count(*) AS result FROM uniusers WHERE username = $1',
		[username],
...
exports.registerUser = function (dbConnection, username, password, permissions, callback) {
	...
	async.waterfall([
			...
			function (hash, sessid, innerCallback) {
				dbConnection.query(
					...
					')',
					[username, salt, hash.toString('hex').substr(0,255), sessid,
						permissions],
					function(error, result) {
...
exports.accessGranted = function(dbConnection, username, password, callback) {
	async.waterfall([
			function (innerCallback) {
				dbConnection.query(
					'SELECT salt, hash FROM uniusers WHERE username = $1',
					[username],
					function(error, result) {
						if (!!result && result.rows.length === 0)
						{
